...
def match_by_tag(
    ...
    default_ = np.zeros((num_kpts, 3 + tag_k.shape[2]))
...
class Params(object):
    def __init__(self, cfg):
        ...
        if cfg.DATASET.WITH_CENTER and not cfg.TEST.IGNORE_CENTER:
            self.joint_order = [
                i - 1
                for i in [18, 1, 2, 3, 4, 5, 6, 7, 12, 13, 8, 9, 10, 11, 14, 15, 16, 17]
            ]
        else:
            self.joint_order = [
                i - 1
...
class MPPEHeatmapParser(object):
    ...
    def match(self, tag_k, loc_k, val_k):
        match = lambda x: match_by_tag(
            x, self.num_kpts, self.max_num_people, self.det_thr, self.tag_thr
        )
    ...
    def top_k(self, det, tag):
        ...
        tag_k = torch.stack(
            [torch.gather(tag[:, :, :, i], 2, ind) for i in range(tag.size(3))], dim=3
        )
    ...
    def parse(self, det, tag, adjust=True, refine=True):
        ...
        scores = np.array([i[:, 2].mean() for i in ans[0]])
